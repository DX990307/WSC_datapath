...
func (t *pageTableImpl) GetLastPage(pid PID) (Page, bool) {
	table := t.getTable(pid)
	table.Lock()
	defer table.Unlock()
	...
	// Get the last page from the list
	elem := table.entries.Back()
	return elem.Value.(Page), true
}
