...
at most 16 concurrent transactions.  Each shader array also has a 16~KB
scalar cache and a 32~KB instruction cache.  The GPM-wide 4~MB L2 is 16-way
associative, accepts 64 requests per cycle, contains 256 MSHRs, and charges a
...
latencies.  CuPath adds one logical metadata-only Cuckoo Filter and bounded
64-line M1 response capacity per GPM, together with bounded local and remote
...
\begin{table}[t]
  ...
  \begin{tabularx}{\columnwidth}{@{}l>{\raggedright\arraybackslash}X@{}}
    ...
    Shared L2 & 4~MB/GPM, 16-way, 256 MSHRs, 64 req./cycle, 10-cycle lookup \\
    ...
    Typed Filter & One logical shared structure/GPM, 32K buckets, four slots/bucket,
                   13-bit fingerprint \\
    Filter ports & 1-cycle lookup/update, 16 parallel lookups and updates/cycle/array \\
    M1 state & 256 observed regions and 64 returned lines/GPM \\
...
We run five configurations with the same benchmark inputs, architectural
parameters, workgroup-completion rule, and completed workgroup count.
...
\cite{balasubramonian2017cacti} at 32~nm. The evaluated GPM contains four
physical Filter arrays with 8K buckets per array. Each bucket has four 21-bit
slots, and each slot contains one valid bit, a 13-bit fingerprint, three type
bits, and a four-bit reference count. One array therefore requires 84~KiB,
and the four arrays require 336~KiB per GPM. We model each array as a 16-bank
scratch RAM with two read ports and one write port per bank. The 256-entry
remote predictor is conservatively budgeted at 64~B per entry, or 16~KiB per
GPM. The Filter arrays and predictor therefore require 352~KiB per GPM, equal
to 8.59\% of the 4-MiB L2 data capacity.

CACTI estimates 0.590~mm$^2$ for each Filter array and 0.036~mm$^2$ for the
remote predictor. The resulting CuPath array area is 2.396~mm$^2$ per GPM. A complete
4-MiB, 16-way L2 including data arrays,
tag arrays, and peripheral circuitry, occupies 54.510~mm$^2$.  CuPath therefore
adds 4.40\% area relative to the modeled L2. Filter and predictor accesses take
0.661~ns and 0.241~ns, respectively, and fit within the 1-ns cycle. The estimate
