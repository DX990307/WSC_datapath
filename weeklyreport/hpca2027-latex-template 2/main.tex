...
propose \emph{CuPath}, which uses typed Cuckoo Filters as a compact control
plane for coordinating memory requests along the local and remote paths.  The
control plane works with three request-processing components: a paired-demand
predictor that identifies adjacent local reads and coarsens HBM accesses, an
RDMA batcher that merges duplicate remote requests and groups requests to the
same home GPM and page, and a remote-reuse policy that retains repeatedly
accessed remote data in the existing requesting-GPM L2.  Exact tags, MSHRs,
and transaction tables remain authoritative, and a request immediately follows
the baseline path when an optimization cannot be applied.  CuPath adds no
global scheduler or new cache level.  Across fourteen workloads, CuPath
achieves a 1.492$\times$ geometric-mean speedup, deduplicates 9.3\% of remote
reads, reduces remote request packets by 52.9\% through aggregation, and eliminates 29.3\% of
repeated wafer traversals.
\end{abstract}
...
reaches the local L2 cache and, on an L2 miss, accesses local HBM.
...
We propose \emph{CuPath}, which uses a lightweight demand predictor and a
logical typed Cuckoo Filter associated with each GPM's L2 cache to optimize requests
...
Across fourteen GPU workloads, CuPath achieves a 1.492$\times$ geometric-mean
speedup and improves 13 workloads, while the remaining workload stays within
0.1\% of Baseline.  More importantly, the speedup follows from work removed
along the path.  CuPath deduplicates 9.3\% of remote reads, reduces remote
request packets by 52.9\% through aggregation, and eliminates 29.3\% of repeated wafer traversals.
...
\begin{itemize}
  ...
  bounded real-demand predictor and logical typed Cuckoo Filter with the
  existing L2 cache, HBM controller, and RDMA engine.

  \item We implement and evaluate CuPath without adding a global scheduler,
  cache level, achieving a 1.492$\times$
