/*
 * heteromark_aes.metal — Metal compute shaders for AES-256 ECB encryption.
 *
 * Each thread encrypts one 16-byte block using AES-256 (14 rounds).
 * Operations: SubBytes (S-box), ShiftRows, MixColumns, AddRoundKey.
 *
 * Buffer layout:
 *   buffer(0): data       — uchar[N*16], plaintext blocks (in/out)
 *   buffer(1): round_keys — uchar[240], expanded round keys (15 * 16 bytes)
 *   buffer(2): params     — uint { num_blocks }
 */

#include <metal_stdlib>
using namespace metal;

// ---------------------------------------------------------------------------
// AES S-Box (256 bytes)
// ---------------------------------------------------------------------------

constant uchar sbox[256] = {
    
};

// ---------------------------------------------------------------------------
// AES helper functions
// ---------------------------------------------------------------------------

static inline uchar xtime(uchar a) {
    return (uchar)((a << 1) ^ (((a >> 7) & 1) * 0x1b));
}

// ---------------------------------------------------------------------------
// AES-256 ECB encrypt kernel — one block per thread
// ---------------------------------------------------------------------------

kernel void aes256_encrypt_kernel(
    device uchar*          data       [[ buffer(0) ]],
    constant uchar*        round_keys [[ buffer(1) ]],
    constant uint&         num_blocks [[ buffer(2) ]],
    uint                   idx        [[ thread_position_in_grid ]])
{
    if (idx >= num_blocks) return;

    // Load 16-byte block into registers
    uchar state[16];
    uint base = idx * 16;
    for (int i = 0; i < 16; ++i)
        state[i] = data[base + i];

    // Initial AddRoundKey
    for (int i = 0; i < 16; ++i)
        state[i] ^= round_keys[i];

    // Rounds 1-13
    for (int r = 1; r <= 13; ++r) {
        // SubBytes
        for (int i = 0; i < 16; ++i)
            state[i] = sbox[state[i]];

        // ShiftRows
        uchar tmp;
        // Row 1: shift left by 1
        tmp = state[1]; state[1] = state[5]; state[5] = state[9];
        state[9] = state[13]; state[13] = tmp;
        // Row 2: shift left by 2
        tmp = state[2]; state[2] = state[10]; state[10] = tmp;
        tmp = state[6]; state[6] = state[14]; state[14] = tmp;
        // Row 3: shift left by 3 (= right by 1)
        tmp = state[15]; state[15] = state[11]; state[11] = state[7];
        state[7] = state[3]; state[3] = tmp;

        // MixColumns
        for (int c = 0; c < 4; ++c) {
            int ci = c * 4;
            uchar a0 = state[ci], a1 = state[ci+1], a2 = state[ci+2], a3 = state[ci+3];
            uchar t = a0 ^ a1 ^ a2 ^ a3;
            state[ci]   = a0 ^ xtime(a0 ^ a1) ^ t;
            state[ci+1] = a1 ^ xtime(a1 ^ a2) ^ t;
            state[ci+2] = a2 ^ xtime(a2 ^ a3) ^ t;
            state[ci+3] = a3 ^ xtime(a3 ^ a0) ^ t;
        }

        // AddRoundKey
        for (int i = 0; i < 16; ++i)
            state[i] ^= round_keys[r * 16 + i];
    }

    // Final round (no MixColumns)
    for (int i = 0; i < 16; ++i)
        state[i] = sbox[state[i]];

    // ShiftRows
    uchar tmp;
    tmp = state[1]; state[1] = state[5]; state[5] = state[9];
    state[9] = state[13]; state[13] = tmp;
    tmp = state[2]; state[2] = state[10]; state[10] = tmp;
    tmp = state[6]; state[6] = state[14]; state[14] = tmp;
    tmp = state[15]; state[15] = state[11]; state[11] = state[7];
    state[7] = state[3]; state[3] = tmp;

    // AddRoundKey (round 14)
    for (int i = 0; i < 16; ++i)
        state[i] ^= round_keys[14 * 16 + i];

    // Store result
    for (int i = 0; i < 16; ++i)
        data[base + i] = state[i];
}
