// heteromark_pagerank.cu — HeteroMark PageRank benchmark (CUDA, self-contained)
//
...
//
// Output (stdout): CSV row — heteromark_pagerank,<N_vertices>,<time_ms>,<edges_per_sec>
...
static void generateCSR(int N, int avgDegree, uint32_t seed,
    ...
    // Build adjacency lists using deterministic PRNG
    // For each vertex u, randomly pick ~avgDegree target vertices
    ...
    for (int u = 0; u < N; ++u) {
        ...
        for (int d = 0; d < deg; ++d) {
            ...
            adj[v].push_back(u);  // edge u→v stored in v's in-neighbor list
        }
    }
    ...
    outDegree.resize(N);
    for (int u = 0; u < N; ++u) {
        outDegree[u] = avgDegree;  // each vertex has exactly avgDegree out-edges
    }

    // Build CSR for incoming edges (for PageRank gather)
    rowPtr.resize(N + 1);
    rowPtr[0] = 0;
    for (int v = 0; v < N; ++v) {
...
// ---------------------------------------------------------------------------
// PageRank kernel: one iteration
//   PR_new[v] = (1-d)/N + d * sum(PR_old[u]/degree[u]) for all u→v
// ---------------------------------------------------------------------------

__global__ void pagerank_kernel(const int* __restrict__ rowPtr,
                                 const int* __restrict__ colIdx,
                                 const int* __restrict__ outDegree,
                                 const float* __restrict__ prOld,
...
int main(int argc, char** argv) {
    ...
    // Host PageRank arrays
    std::vector<float> h_pr(N, 1.0f / (float)N);
    std::vector<float> h_prNew(N);

    // Device allocations
    int *d_rowPtr, *d_colIdx, *d_outDegree;
    float *d_prOld, *d_prNew;

    CUDA_CHECK(cudaMalloc(&d_rowPtr, (N + 1) * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_colIdx, numEdges * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_outDegree, N * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_prOld, N * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_prNew, N * sizeof(float)));

    // Copy graph to device (constant across runs)
    CUDA_CHECK(cudaMemcpy(d_rowPtr, rowPtr.data(), (N + 1) * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_colIdx, colIdx.data(), numEdges * sizeof(int), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_outDegree, outDegree.data(), N * sizeof(int), cudaMemcpyHostToDevice));

    int blockSize = 256;
    int gridSize  = (N + blockSize - 1) / blockSize;

    // Lambda: run full PageRank (prIters iterations)
    auto run_pagerank = [&]() {
        // Initialize PR to 1/N
        for (int i = 0; i < N; ++i) h_pr[i] = 1.0f / (float)N;
        CUDA_CHECK(cudaMemcpy(d_prOld, h_pr.data(), N * sizeof(float), cudaMemcpyHostToDevice));

        for (int it = 0; it < prIters; ++it) {
            pagerank_kernel<<<dim3(gridSize), dim3(blockSize), 0, 0>>>(d_rowPtr, d_colIdx, d_outDegree, d_prOld, d_prNew, N, damping);
    ...
    // -------------------------------------------------------------------
    CUDA_CHECK(cudaMemcpy(h_pr.data(), d_prOld, N * sizeof(float), cudaMemcpyDeviceToHost));
    ...
    int errors = 0;
    for (int v = 0; v < N; ++v) {
        float diff = fabsf(h_pr[v] - refPr[v]);
    ...
    CUDA_CHECK(cudaFree(d_prOld));
    CUDA_CHECK(cudaFree(d_prNew));
