// rodinia_kmeans.cu — Rodinia K-Means Clustering benchmark (CUDA, self-contained)
//
...
//
// Output (stdout): CSV row — kmeans,N,K,D,<iterations>,<time_ms>,<Gpoints_per_s>
// Output (stderr): Per-iteration delta, convergence status, PASS/FAIL
...
// ---------------------------------------------------------------------------
// Kernel 1: Assign each point to nearest cluster center
//
//   points[idx * D + d] — row-major: N x D
//   centers[k  * D + d] — row-major: K x D
//   membership[idx]     — output cluster index for each point
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Kernel 2: Accumulate new cluster centers using atomics
//
//   new_centers[k * D + d] — running sum (zeroed before each call)
//   counts[k]              — number of points in cluster k
// ---------------------------------------------------------------------------
    ...
    int N, int D)
{
    ...
    for (int d = 0; d < D; ++d) {
        atomicAdd(&new_centers[k * D + d], points[idx * D + d]);
    }
}
...
int main(int argc, char** argv)
{
    ...
    float* h_centers     = (float*)malloc(centers_bytes);
    float* h_new_centers = (float*)malloc(centers_bytes);
    int*   h_counts      = (int*)malloc((size_t)K * sizeof(int));
    int*   h_membership  = (int*)malloc((size_t)N * sizeof(int));
    int*   h_membership_prev = (int*)malloc((size_t)N * sizeof(int));

    // Initialize points randomly in [0, 1)^D
    ...
    CUDA_CHECK(cudaMalloc(&d_new_centers, centers_bytes));
    CUDA_CHECK(cudaMalloc(&d_counts,      (size_t)K * sizeof(int)));
    CUDA_CHECK(cudaMalloc(&d_membership,  (size_t)N * sizeof(int)));

    CUDA_CHECK(cudaMemcpy(d_points,  h_points,  points_bytes,  cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_centers, h_centers, centers_bytes, cudaMemcpyHostToDevice));
    ...
    for (int w = 0; w < num_warmup; ++w) {
        kmeans_assign_kernel<<<dim3(grid_size), dim3(block_size), 0, 0>>>(d_points, d_centers, d_membership, N, D, K);
    ...
    for (iter = 0; iter < max_iter; ++iter) {
        ...
        CUDA_CHECK(cudaMemset(d_new_centers, 0, centers_bytes));
        CUDA_CHECK(cudaMemset(d_counts,      0, (size_t)K * sizeof(int)));

        // --- Step 3: accumulate ---
        kmeans_update_kernel<<<dim3(grid_size), dim3(block_size), 0, 0>>>(d_points, d_new_centers, d_counts, d_membership, N, D);

        // --- Step 4: copy back and divide on CPU ---
        CUDA_CHECK(cudaMemcpy(h_new_centers, d_new_centers, centers_bytes,   cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(h_counts,      d_counts,      (size_t)K * sizeof(int), cudaMemcpyDeviceToHost));
        CUDA_CHECK(cudaMemcpy(h_membership,  d_membership,  (size_t)N * sizeof(int), cudaMemcpyDeviceToHost));
        ...
        for (int k = 0; k < K; ++k) {
            if (h_counts[k] > 0) {
                ...
                memcpy(&h_new_centers[k * D], &h_centers[k * D], (size_t)D * sizeof(float));
            }
        }
        ...
        // Update centers on device
        memcpy(h_centers, h_new_centers, centers_bytes);
        CUDA_CHECK(cudaMemcpy(d_centers, h_centers, centers_bytes, cudaMemcpyHostToDevice));

        memcpy(h_membership_prev, h_membership, (size_t)N * sizeof(int));
    ...
    float time_ms = 0.0f;
    CUDA_CHECK(cudaEventElapsedTime(&time_ms, ev_start, ev_stop));
    ...
    // Pass if delta dropped vs first iteration (convergence is occurring),
    // or if delta < 5% (acceptable for benchmarks with few iterations).
    ...
    // Report
    // -----------------------------------------------------------------------
    double gpoints_per_s = (double)N * (double)iter / ((double)time_ms * 1e-3) / 1e9;
    ...
    CUDA_CHECK(cudaFree(d_new_centers));
    CUDA_CHECK(cudaFree(d_counts));
    CUDA_CHECK(cudaFree(d_membership));
