...
type Operator interface {
	Create(size []int, gpuid int) Tensor
	...
	Transpose(t Tensor, order []int, executeGPUID int, saveGPUID int) (Tensor, *driver.CommandQueue)
	Rotate180(t Tensor) Tensor
	Dilate(t Tensor, dilate []int) Tensor

	Sum(t Tensor, axis []int) Tensor

	Gemm(transA, transB bool, alpha, beta float64, a, b, c Tensor, executeGPUID int, saveGPUID int) (Tensor, []*driver.CommandQueue)
	Im2Col(t Tensor, kernelSize, padding, stride, dilation []int, executeGPUID int, saveGPUID int) (Tensor, *driver.CommandQueue)
