...
func (o *GPUOperator) Create(size []int, gpuid int) tensor.Tensor {
	t := &Tensor{
		driver: o.driver,
		ctx:    o.ctx,
		size:   size,
...
func (o *GPUOperator) Repeat(t tensor.Tensor, times int) tensor.Tensor {
	...
	out := o.Create([]int{numElem * times}, 1).(*Tensor)
...
func (o *GPUOperator) Transpose(t tensor.Tensor, order []int, executeGPUID int, saveGPUID int) (tensor.Tensor, *driver.CommandQueue) {
	o.driver.SelectGPU(o.ctx, executeGPUID)
	...
	output := o.Create(outSize, saveGPUID).(*Tensor)
	...
	fmt.Printf("Transpose: start time: %0.9f", o.driver.Engine.CurrentTime())
	o.timerEnd("Transpose")
	o.driver.EnqueueLaunchKernel(
		queue,
		o.transposeKernel,
		[3]uint32{uint32(t.NumElement()), 1, 1},
		[3]uint16{64, 1, 1},
...
func (o *GPUOperator) Rotate180(t tensor.Tensor) tensor.Tensor {
	...
	output := o.Create(outSize, 1).(*Tensor)
...
func (o *GPUOperator) Gemm(
	...
	executeGPUID int,
	saveGPUID int,
) (tensor.Tensor, []*driver.CommandQueue) {
	o.driver.SelectGPU(o.ctx, executeGPUID)
	tempA := a
	queuelist := make([]*driver.CommandQueue, 3)
	if transA {
		tempA, queuelist[0] = o.Transpose(a, []int{1, 0}, executeGPUID, saveGPUID)
	}

	tempB := b
	if transB {
		tempB, queuelist[1] = o.Transpose(b, []int{1, 0}, executeGPUID, saveGPUID)
	}

	d, queue := o.matrixMultiplication(alpha, beta, tempA, tempB, c, executeGPUID, saveGPUID)
	queuelist[2] = queue
...
func (o *GPUOperator) matrixMultiplication(
	...
	// 	&kernArg,
	// )
	fmt.Printf("Gemm: start time: %0.9f", o.driver.Engine.CurrentTime())
	o.driver.EnqueueLaunchKernel(
		queue,
		o.gemmKernel,
		[3]uint32{uint32(wiWidth), uint32(wiHeight), 1},
...
func (o *GPUOperator) Im2Col(
	...
	executeGPUID int, saveGPUID int,
) (tensor.Tensor, *driver.CommandQueue) {
	o.driver.SelectGPU(o.ctx, executeGPUID)
	queue := o.driver.CreateCommandQueue(o.ctx)
	inputSize := t.Size()
	...
	output := o.Create([]int{outHeight, outWidth}, saveGPUID)
	...
	// 	&kernArg,
	// )
	fmt.Printf("Im2Col: start time: %0.9f", o.driver.Engine.CurrentTime())
	o.driver.EnqueueLaunchKernel(
		queue,
		o.im2ColKernel,
		[3]uint32{uint32(outWidth), uint32(outHeight), 1},
...
func (o *GPUOperator) MaxPoolingForward(
	...
	outT := o.Create([]int{n, c, hOut, wOut}, 1).(*Tensor)
	maskT := o.Create([]int{n, c, hOut, wOut}, 1).(*Tensor)
...
func (o *GPUOperator) MaxPoolingBackward(
	...
	out := o.Create([]int{n, c, hIn, wIn}, 1)
...
func (o *GPUOperator) AvgPoolingForward(
	...
	output := o.Create([]int{B, C, Hout, Wout}, 1).(*Tensor)
...
func (o *GPUOperator) AvgPoolingBackward(
	...
	output := o.Create([]int{B, C, Hin, Win}, 1).(*Tensor)
...
func (o *GPUOperator) Softmax(t tensor.Tensor) tensor.Tensor {
	...
	input := t.(*Tensor)
	output := o.Create(input.size, 1).(*Tensor)
	expInput := o.Create(
		[]int{input.size[0], t.NumElement() / input.size[0]}, 1,
	).(*Tensor)
...
func (o *GPUOperator) CrossEntropyDerivative(
	...
	output := o.Create(t.Size(), 1).(*Tensor)
...
func (o *GPUOperator) ReluBackward(
	...
	out := o.Create(forwardIn.Size(), 1).(*Tensor)
