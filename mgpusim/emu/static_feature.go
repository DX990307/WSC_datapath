...
var Static_compute_units map[uint64]*StaticComputeUnit

func StaticComputeUnitForGPU(gpuID uint64) *StaticComputeUnit {
	if Static_compute_units != nil {
		if cu := Static_compute_units[gpuID]; cu != nil {
			return cu
		}
	}
	return Static_compute_unit
}
...
func CreateStaticComputeUnitForGPU(
	gpuID uint64,
	name string,
	freq sim.Freq,
	decoder Decoder,
	...
	if Static_compute_units == nil {
		Static_compute_units = make(map[uint64]*StaticComputeUnit)
	}
	Static_compute_units[gpuID] = BuildStaticComputeUnit(
		name,
		freq,
		decoder,
		pageTable,
